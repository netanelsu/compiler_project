    ...
    if(tree){
        ...
        if(idType(tree->token)==3){
            ...
            if(tree->mid2){
                ...
                if(strcmp(retype,getMyFuncReturn(**scope1))){
                    if((!strcmp(retype,"void") && !strcmp((*scope1)->scope_id,"BODY")) ||(!strcmp(retype,"void") && !strcmp((*scope1)->scope_id,"IF")) || (!strcmp(retype,"void") && !strcmp((*scope1)->scope_id,"WHILE") || (!strcmp(retype,"void") && !strcmp((*scope1)->scope_id,"ELSE")))){
        ...
        if(idType(tree->token)==4){
            ...
            if(idType(tree->left->token)==2){
                    if(tree->left->mid2)
                       getExprType(*tree->left->mid2,**scope1);
                    ...
                    if((*scope1)->vars){
                     ...
                     var* tmp = (*scope1)->params;
                       if (tmp){
                            while(tmp){
                                if (!strcmp(tree->left->mid1->left->token,tmp->id)){
                                    char* error=(char*)malloc(sizeof(char)*100);
                                    strcpy(error,"Variable '");
                                    strcat(error,tree->left->mid1->left->token);
                                    strcat(error,"' already declared");
                                    printError(error);
                                }
                                else tmp = tmp->next;
        ...
        if(idType(tree->token)==6){
            ...
            if(idType(tree->left->token)==1){
                ...
                if(strcmp(getExprType(*tree->left->left,*(*scope1)),"bool")){
                     char* error=(char*)malloc(sizeof(char)+50);
                        strcpy(error,tree->left->token);
                        strcat(error," condition is '");
                        strcat(error,getExprType(*tree->left->left,**scope1));
                        strcat(error,"' instead of '");
                        strcat(error," bool ");
                        strcat(error,"', check your '");
                        strcat(error,(*scope1)->scope_id);
...
int idType(char* id){
    if(!strcmp(id,"FUNCTION") || !strcmp(id,"PROCEDURE"))
        return 0;
    if(!strcmp(id,"IF") || !strcmp(id,"WHILE"))
        return 1;
    if(!(strcmp(id,"IDS")))
        return 2;
    if(!(strcmp(id,"BODY")))
...
char* getExprType(node tree,scope scp){
    ...
    if(idType(tree.token)==12 || idType(tree.token) == 20 ){
        //&
        if(tree.mid2){
        ...
        if(tree.left && idType(tree.token) == 20 ){
            if(strcmp(getExprType(*tree.left,scp),"int"))
                    printError("Index must be int");
    ...
    if(idType(tree.token)==15){
       ...
       if(idType(tree.token)==16){
        ...
        if(!strcmp(getExprType(*(tree.left),scp),"int*") || !strcmp(getExprType(*(tree.left),scp),"int*")){
            ...
            printError("operator '^' is only compatible with pointers");
            return NULL;
        }
