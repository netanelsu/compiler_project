...
%token IF ELSE WHILE RRETURN NNULL;

%token AND DIVIDE ASSIGN ISEQUAL GREATER GREATEREQ LESS LESSEQ MINUS NOT NOTEQUAL OR PLUS MULT BAND DEREFERENCE;

%token BOOL_TRUE BOOL_FALSE FUNC PROC;
...
*/
PROGRAM: FUNCTIONS { $$=mknode("PROGRAM",$1,NULL ,NULL,NULL);
					strcat($$->code,$1->code);
					};							
						
						
FUNCTIONS: FUNCTION FUNCTIONS{ $$ = mknode("FUNC_NODE",$1,$2,NULL,NULL);
								strcat($$->code,$1->code);	
								strcat($$->code,"\n");
								if($2 != NULL)
									strcat($$->code,$2->code);
...
/*
FUNC_TYPE can be boolean , char , int , charp , intp .
The body can contain variable declarations, statements and return statment in this order only. */					
FUNCTION:				FUNC id OBRACKET F_PARAMS CBRACKET RRETURN TYPE CODE_BLOCK {
						char num[33];
						$$=mknode("FUNCTION",$7,$2,$4,$8);
						strcat($$->code,$2->token);
							strcat($$->code,": \n\tBeginFunc ");
							strcat($$->code,"\n");
							strcat($$->code,$8->code);	
						...
						|PROC id OBRACKET F_PARAMS CBRACKET CODE_BLOCK {
							char num[33];
							$$=mknode("PROCEDURE",mknode("void",NULL,NULL,NULL,NULL),$2,$4,$6);
							strcat($$->code,$2->token);
							strcat($$->code,": \n\tBeginFunc ");
							strcat($$->code,"\n");
							strcat($$->code,$6->code);
...
*/
PARAM:					 IDS ':' PARAM_TYPE {$$=mknode($3->token,$1,NULL,NULL,NULL);}
						;
						

VAR_DECLERACTION:		VAR IDS ':' TYPE SEMI {$$=mknode("IDS",$4,$2,NULL,NULL);}
						| VAR IDS ':' string SID SEMI {$$=mknode("IDS",$4,$2,$5,NULL);}
						;
...
SID:					OSQRERACKET EXPR CSQRERACKET {$$=mknode("SIZE",$2,NULL,NULL,NULL);}
						;
...
ASSIGNMENT:				ASSIGNMENT_VAR assign EXPR SEMI{
                        ...
                        | ASSIGNMENT_VAR assign BAND id OSQRERACKET EXPR CSQRERACKET SEMI{
							...
							strcat($$->code," = ");
							strcat($$->code,$6->var);}
						| DEREFERENCE ASSIGNMENT_VAR assign EXPR SEMI{
...
ASSIGNMENT_VAR:			  id {$$ = $1;};
						| id OSQRERACKET EXPR CSQRERACKET {
							$$=mknode($1->token,mknode("INDEX",$3,NULL,NULL,NULL),NULL,NULL,NULL);
							$$->isArray = 1;
						...
						| DEREFERENCE id{$$=mknode("^",$2,NULL,NULL,NULL);}
						| DEREFERENCE OBRACKET EXPR CBRACKET {$$=mknode("^",$3,NULL,NULL,NULL);};
						;
...
IFS:					IF OBRACKET EXPR CBRACKET CODE_BLOCK{
							...
							while(flag){
								...
								tmp = tmp->mid1;
								if(strcmp(tmp->token,"||") && strcmp(tmp->token,"&&")){
									flag = 0;
								}
						...
						| IF OBRACKET EXPR CBRACKET SINGLE_STATMENT ELSE SINGLE_STATMENT { 
							$$=mknode("IF",$3,$5,$7,NULL);
							$$->begin = freshLabel();
							$$->next = freshLabel();
							char* label = freshLabel();
...
EXPR_LIST:				EXPR COMMA EXPR_LIST { 
						...
						| EXPR{
							$$=mknode("VARIABLES",$1,NULL,NULL,NULL);
							strcat($$->code,$1->code);
							strcat($$->code, "\tPushParams ");
							strcat($$->code, $1->var);
							strcat($$->code, "\n"); 
...
EXPR:					EXPR PLUS EXPR {
						...
						| NUMS {
							...
							basicOperationCode(&($$->code),"","",$$->var,$1->token,"","");
							}
						| BOOL_TRUE {
